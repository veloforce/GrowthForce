...
from .interact_state import parse_collect_button_state, parse_like_button_state
...
_GET_COLLECT_BUTTON_STATE_JS = f"""
(() => {{
    const el = document.querySelector({json.dumps(COLLECT_BUTTON)});
    ...
    for (let i = 0; cur && i < 4; i += 1, cur = cur.parentElement) {{
        nodes.push({{
            ...
            ariaPressed: cur.getAttribute("aria-pressed"),
            ariaLabel: cur.getAttribute("aria-label"),
            href: cur.querySelector("use")?.getAttribute("xlink:href") || cur.querySelector("use")?.getAttribute("href"),
            title: cur.getAttribute("title"),
            text: String(cur.innerText || cur.textContent || "")
        }});
    }}
    return JSON.stringify({{"exists": true, "nodes": nodes}});
}})()
"""

_GET_LIKE_BUTTON_STATE_JS = f"""
(() => {{
    const el = document.querySelector({json.dumps(LIKE_BUTTON)});
    if (!el) return JSON.stringify({{"exists": false}});
    const nodes = [];
    let cur = el;
    for (let i = 0; cur && i < 4; i += 1, cur = cur.parentElement) {{
...
def _get_collect_button_state(page: Page) -> bool | None:
    """读取收藏按钮 DOM 状态。"""
    try:
        raw = page.evaluate(_GET_COLLECT_BUTTON_STATE_JS)
        if not raw:
            return None
        parsed = json.loads(raw)
    except Exception as error:
        logger.debug("读取收藏按钮 DOM 状态失败: %s", error)
        return None
    return parse_collect_button_state(parsed)


def _get_like_button_state(page: Page) -> bool | None:
    """读取点赞按钮 DOM 状态。"""
    try:
        raw = page.evaluate(_GET_LIKE_BUTTON_STATE_JS)
        if not raw:
            return None
        parsed = json.loads(raw)
    except Exception as error:
        logger.debug("读取点赞按钮 DOM 状态失败: %s", error)
        return None
    return parse_like_button_state(parsed)
...
def _toggle_like(page: Page, feed_id: str, target_liked: bool) -> ActionResult:
    ...
    try:
        ...
        button_state = _get_like_button_state(page)
    ...
    try:
        ...
        pass
    if _get_like_button_state(page) == target_liked:
        logger.info("feed %s %s成功", feed_id, action_name)
        return ActionResult(feed_id=feed_id, success=True, message=f"{action_name}成功")
...
def _wait_collected_state(
    ...
    while time.monotonic() < deadline:
        ...
        button_state = _get_collect_button_state(page)
        if button_state == target_collected:
            return True
        time.sleep(interval)
    return False
