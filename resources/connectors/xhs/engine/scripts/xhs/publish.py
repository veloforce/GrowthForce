...
from .publish_result import PUBLISH_RESULT_TIMEOUT_SECONDS, resolve_publish_result
from .publish_preflight import extract_final_tags
...
def publish_image_content(page: Page, content: PublishImageContent) -> dict:
    """发布图文内容（填写表单 + 点击发布）。
    ...
    return click_publish_button(page)
...
def click_publish_button(page: Page) -> dict:
    """触发发布。
    ...
    """
    # 1) CDP Network 是主证据。它不受页面覆盖 fetch/XHR、Service Worker 或跳转销毁
    #    JavaScript 上下文影响。
    ...
    page.evaluate(
        ...
        (() => {
            ...
            function looksLikePublishUrl(url) {
                return /publish|create[_/-]?note|note[_/-]?(create|publish)|\\/web_api\\/sns\\/v2\\/note/i.test(url || '');
            }

            // 成功响应含 note_id；发布类请求的任意业务码都必须被判定。
            function looksLikePublishResp(url, body) {
                if (!body) return false;
                return body.includes('HTTPBizError')
                    || body.includes('禁止发笔记')
                    || /"note_id":\\s*"[^"]+"/.test(body)
                    || (looksLikePublishUrl(url)
                        && (/"code":\\s*-?\\d+/.test(body)
                            || /"result":\\s*-?\\d+/.test(body)
                            || /"success":\\s*(true|false)/.test(body)));
            }
            ...
            function captureFromBody(source, url, status, body) {
                ...
                if (!looksLikePublishResp(url, body)) return;
                try {
            ...
            const RISK_KW = /违反|违规|禁止发笔记|审核未通过|账号异常/;
            const FAILURE_KW = /无法发布|发布失败/;
            const obs = new MutationObserver((muts) => {
                ...
                muts.forEach((m) => {
                    m.addedNodes.forEach((n) => {
                        ...
                        if (!RISK_KW.test(txt) && !FAILURE_KW.test(txt)) return;
                        capture('toast', {
                            code: RISK_KW.test(txt) ? -9136 : -1,
                            msg: txt,
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if request_id:
                ...
                if failed:
                    error_text = failed.get("params", {}).get("errorText", "unknown")
                    raise PublishError(f"发布请求网络失败：{error_text}")
                try:
                    page.wait_for_event(
                        "Network.loadingFinished",
                        predicate=lambda item: item.get("params", {}).get("requestId")
                        == request_id,
                    ...
                    if failed:
                        error_text = failed.get("params", {}).get("errorText", "unknown")
                        raise PublishError(f"发布请求网络失败：{error_text}")
                    # 小响应通常在 responseReceived 后已可读取；事件丢失时继续尝试响应体。
                body_result = page._send_session(
            ...
            logger.warning("解析发布 Network 响应失败: %s", error)
        ...
        if result:
            break
        try:
            published_url = page.evaluate("location.href")
        except Exception:
            published_url = ""
        if "published=true" in (published_url or ""):
            logger.warning("页面已进入 published=true，但未取得发布响应体")
            from .errors import PublishResultTimeoutError

            raise PublishResultTimeoutError("published_redirect_without_response")

    # 5) 解析业务码
    if not result:
        logger.warning(
            "%ss 内未捕获到任何发布反馈（XHR/console/toast 都没匹配）",
            PUBLISH_RESULT_TIMEOUT_SECONDS,
        )
        return resolve_publish_result(None)

    source = result.get("source", "unknown")
    code = result.get("code", result.get("result"))
    msg = result.get("msg", "")
    logger.info("捕获发布响应（来源=%s code=%s msg=%r）", source, code, msg)
    resolved = resolve_publish_result(result)
    logger.info("发布成功: note_id=%s", resolved["note_id"])
    time.sleep(2)
    return resolved
...
def _upload_images(page: Page, image_paths: list[str]) -> None:
    ...
    for i, path in enumerate(valid_paths):
        selector = _mark_current_image_input(page)
        logger.info("上传第 %d 张图片: %s", i + 1, path)

        page.set_file_input(selector, [])
        ...
        state = page.evaluate(
            ...
            (() => {{
                const errorText = [...document.querySelectorAll(
                    '.error, .error-text, .upload-error, [class*="upload"][class*="error"]'
                )].map((el) => el.textContent || '').join(' ').trim();
                return {{errorText}};
            }})()
            """
        )
        if state and state.get("errorText"):
            raise PublishError(f"图片上传失败：{state['errorText'][:200]}")
        time.sleep(0.5)

    raise UploadTimeoutError(f"第{expected_count}张图片上传超时(60s)")


def _mark_current_image_input(page: Page) -> str:
    """标记当前图文面板的真实图片 input，排除视频和明显 honeypot。"""
    selector = '[data-agentstudio-image-upload="1"]'
    found = page.evaluate(
        f"""
        (() => {{
            document.querySelectorAll({json.dumps(selector)}).forEach((el) => {{
                el.removeAttribute('data-agentstudio-image-upload');
            }});
            const candidates = [...document.querySelectorAll('input[type="file"]')];
            for (const input of candidates) {{
                const accept = (input.getAttribute('accept') || '').toLowerCase();
                if (!/(jpg|jpeg|png|webp|image)/.test(accept)) continue;
                if (input.closest('[data-hp-kind], [button-hp-installed]')) continue;
                const container = input.closest(
                    '.upload-content, [class*="upload"], [class*="image"]'
                );
                if (container && getComputedStyle(container).display === 'none') continue;
                input.setAttribute('data-agentstudio-image-upload', '1');
                return true;
            }}
            return false;
        }})()
    ...
    if not found:
        raise PublishError("未找到当前图文面板的图片上传控件")
    return selector


def _consume_network_event(
    page: Page,
    method: str,
    request_id: str,
    *,
...
def _extract_hashtags_from_content(content: str, tags: list[str]) -> tuple[str, list[str]]:
    """兼容旧调用点，实际规则由本地预检模块统一维护。"""
    cleaned, merged = extract_final_tags(content, tags)
    if cleaned != content or merged != tags:
        logger.info("从正文末尾提取并合并标签，最终共 %d 个", len(merged))
