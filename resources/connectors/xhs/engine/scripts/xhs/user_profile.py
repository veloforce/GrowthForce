...
def get_user_profile(page: Page, user_id: str, xsec_token: str = "") -> UserProfileResponse:
    ...
    Args:
        page: CDP 页面对象。
        user_id: 用户 ID。
        xsec_token: 可选 xsec_token。用户主页可直接通过 user_id 访问；
            该参数仅兼容从笔记上下文进入主页的旧入口。
...
def _wait_for_user_notes(page: Page, timeout: float = 8.0) -> None:
    """等待主页 notes 数据有机会完成加载。"""
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        has_notes = page.evaluate(
            ...
            (() => {
                const notes = window.__INITIAL_STATE__?.user?.notes;
                const data = notes && notes.value !== undefined ? notes.value : notes?._value;
                if (!Array.isArray(data)) return false;
                return data.some((group) => Array.isArray(group) ? group.length > 0 : !!group);
            })()
            """
        )
        if has_notes:
            return
        time.sleep(0.5)
    logger.warning("等待 user.notes 非空超时")
