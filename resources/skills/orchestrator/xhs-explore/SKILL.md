...
- 只读搜索和分析小红书公开内容、主页、评论与当前账号可用的创作者数据。
- 不执行发布、评论、回复、点赞或收藏，不修改账号长期数据。

## 输入与输出

- 输入：搜索关键词、目标笔记或用户标识、筛选条件、数据范围和本轮已选账号。
- 输出：CLI 实际返回的内容、指标、来源范围、权限缺口与失败原因。

## 授权

本 Skill 全程只读，不产生平台外部副作用，无需发布或互动授权；用户要求真实互动时必须交给
`xhs-interact`。
...
完成后将研究数据交给账号诊断、内容研究、策略、采集或互动阶段；本 Skill 不自行推进外部动作。
