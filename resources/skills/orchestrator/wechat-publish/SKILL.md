...
- 消费已校验发布包并推送到当前公众号草稿箱，记录平台结果并接线发布后采集。
- 不正式发布公众号文章，不生成或改写正文，不读取或回复留言。

## 输入与输出

- 输入：非空标题、正文 HTML 绝对路径、封面绝对路径、摘要、可选作者和上游 runId。
- 输出：`media_id`、最终内容摘要、图片上传结果、warnings、推送时间与失败原因。

## 授权

- 当前能力只推送公众号草稿箱，不属于正式对外发布，因此手动和自动化任务均无需发布授权。
- 若未来支持公众号正式发布，必须由独立的正式发布 Skill 获取授权，不能复用本例外。
...
- 输入或依赖不完整时停止并保留发布包，不在本 Skill 内补写内容。
- 推送失败时不得标记 publication 完成或创建采集任务。
- 推送成功且存在 runId 时交给采集阶段；没有 runId 时只返回草稿箱记录。
