---
name: xhs-create
agent: orchestrator
description: |
  小红书内容生成技能，负责生成并校验标题、正文、标签和视觉素材发布包。
  当用户要求创作小红书笔记、改写为小红书内容或准备发布素材时触发。
version: 1.1.0
---

# 小红书内容生成

本 Skill 负责生成发布包，不直接调用小红书发布或互动 CLI 命令。

## 职责

- 生成并校验小红书标题、正文、标签和视觉素材，形成标准发布包与可选 runId。
- 不执行发布或互动，不负责发布授权、发布后采集和长期复盘。

## 输入与输出

- 输入：用户目标或 Content Brief、已选账号、Profile、适用 Playbook、近期 Run、research 和素材。
- 输出：符合平台硬约束的完整发布包，以及完整运营任务对应的 runId。

## 账号定位 Gate 与长期数据规则

内容生成前需要读取账号长期数据作为输入
- 账号定位
  - `content_profile_get` ，最高优先级,Profile 是账号级事实，不得根据账号昵称、用户身份、本轮主题或历史对话自行推断定位后继续创作
  - Profile 已存在且足以完成本轮任务时，按 Profile 约束生成。Profile 缺失或缺少本轮目标必需
    字段时，前台对话交给 `account-profile-ops` 只补充必要信息；自动化任务跳过补充并说明
    缺失对本次创作的影响。

- 创作经验
  - `content_playbook_read` 已验证规律，参考但不得违背 Profile
  - `content_runs_recent` 最近发布与待复盘状态，用于避免重复选题

- 用户偏好
  - 用户明确提出可长期复用的创作偏好、结构要求、风格要求、禁区或 CTA 习惯时
  - 先用`content_playbook_preferences_read` 读取「用户明示偏好」区
  - 再用`content_playbook_preferences_replace` 整段替换该区, 不得写入表现判断或数据规律
  

## 内容创作要求

平台规则参考
- 先读取 `references/platform-rules.md`，将其中的官方硬约束作为发布包校验规则；
- 非官方经验只能作为优化建议，不得表述为确定的平台算法或激励承诺。

内容创作参考
- 可以参考 references/xhs-create-reference.md


## Run 记录与初稿快照
本 Skill 只创建本次 Run 并写 `draft` 快照；不写 History、证据规律、metrics 或 review，不创建发布后采集任务。
Run 是完整运营任务的记录层，由本 Skill 作为生产起点创建并持有 runId：

- 只要本轮产出可发布草稿或发布包，必须用 `content_run_create_with_draft`（传入本轮目标、当前工作目录和初稿 Markdown）创建 Run，并记下 runId。
- 该工具会把初稿写成**不可变快照**，并将 `content_generation` 阶段置为 `completed`
- 该快照供后续复盘做版本对比，与用户在工作目录里继续编辑的工作文件相互独立、互不影响。
- 只有局部润色、标题备选、结构建议等不形成可发布草稿/发布包的任务，才允许不创建 Run。


## 输出发布包

- 标题：按小红书 UTF-16 计数规则不超过 20 单位。ASCII 字符计 0.5 单位并向上取整，
  非 ASCII 按 UTF-16 code unit 计 1 单位；emoji、旗帜和 ZWJ 序列可能占多个单位。
  生成后必须按该规则自检；超出时保持核心语义重新创作到 19–20 单位，禁止机械截断。
- 正文：不超过 800 字，短段落、自然表达、明确价值和互动引导；超出需先压缩再产出发布包。
- 标签：与内容和目标用户匹配，CLI 参数和正文末尾 hashtag 合并、去重后不超过 6 个。
- 素材：图文提供 1-6 张图片，视频内容提供视频绝对路径。
- 发布目标：目标用户、核心卖点、预期互动点和后续复盘指标。
- runId：发布包交给 `xhs-publish` 时一并把 runId 传递下去。
- 图片可以是存在的绝对路径或 URL 模式提取出的 HTTP/HTTPS 地址，并标明素材来源为用户提供、网页提取、远程生成或本地模板。
- 图文最多生成或使用 6 张图片。
- 视频发布包不调用图片生成工具，也不生成兜底图片。


## 授权与发布交接

当用户目标包含发布、完整运营闭环或自动化发布时，生成完整发布包后必须立即交给 `xhs-publish`，不得返回发布包后自动停止。
只有用户明确要求 “只生成草稿/只生成内容/不发布”时，才停止。

本 Skill 只生成、校验并传递完整发布包和 runId，不负责发布前确认：
- 发布前确认是由`xhs-publish`负责。
- 针对发布确认不得调用 `AskUserQuestion`。
- 不得用普通文字询问用户是否确认发布。
- 不得在交接前以 `end_turn` 停止。

## 失败与交接

- Profile、素材或平台必需字段不足时保留现有产物并说明缺口，不生成伪造占位内容。
- 用户要求发布、完整流程或自动化发布时，发布包校验通过后立即交给 `xhs-publish`。
- 用户明确只要草稿、内容、发布包或不发布时，以发布包结束，不触发授权。
