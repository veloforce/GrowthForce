...
**规则优先级**：账号定位 > 生命周期长期策略 > persona > 本文件。长期策略必须来自
`content_playbook_read` 返回的有证据规则，执行层不更新长期策略。
...
**关于 H1**：必须写 H1 标题（20-28 个中文字）。`wechat-markdown-to-html` 会自动把 H1 提取为微信的标题字段，并从正文 HTML 中移除。所以 Markdown 里写 H1，微信里看到的是独立标题栏 + 从 H2 开始的正文。
