...
- 消费完整发布包，执行小红书发布并记录实际平台结果和发布后采集接线。
- 负责小红书发布的唯一授权、风控与结果未知处理；不生成或改写发布内容。

## 输入与输出

- 输入：已选账号、完整发布包、发布设置和可选 runId；回补模式另需既有尝试标识。
- 输出：实际发布记录、平台标识与时间、Run 阶段状态、采集任务状态或明确失败原因。

## 授权

- 本 Skill 是小红书发布授权的唯一执行方；`xhs-create` 不得提前询问确认。
- 手动对话完成发布包校验和风控检查后，必须调用 `AskUserQuestion` 展示最终标题、正文、标签、
  素材、定时与可见性，获取唯一一次结构化确认；`risk_level == high` 时，高风险原因与风险摘要
  合并到这一次最终发布确认，默认/推荐选项必须是“停止发布”，另一个选项是“仍要继续发布”；
  不得在最终发布确认前单独调用 `AskUserQuestion`。取消时不得填写表单或执行发布。
- 自动化任务跳过交互确认，但只能使用任务描述、附件或上游产物中明确给定且预授权的发布包；
  输入不完整或安全条件不满足时直接失败。
- 填表成功后直接发布，不得再次请求浏览器预览确认。
...
2. 若 `risk_level == high` 且本轮运行上下文是自动化任务，直接停止发布，
   返回机器可读原因 `XHS_RISK_HIGH_BEFORE_PUBLISH`；不得调用 `fill-publish`、`click-publish`
   或 `save-draft`，也不得等待用户确认。
3. 若 `risk_level == high` 且本轮是手动对话，不单独询问风险是否继续；保留高风险原因和风险
   摘要，继续进入步骤 6 的唯一一次最终发布确认。
4. 若 `risk_level == medium`，手动对话继续进入最终发布确认，但确认文案必须附带风险摘要；
   自动化任务默认继续，但不做额外重试。
5. 若风控诊断不可用但主流程继续，记录机器可读原因 `XHS_RISK_OBSERVATION_UNAVAILABLE`。
6. 展示最终标题、正文、标签、图片或视频、定时和可见性设置，并调用 `AskUserQuestion`
   获取唯一一次结构化确认；不得只用普通文字回复“请确认”，否则本轮会停止导致流程中断。
   - `risk_level == high`：同时展示高风险原因和风险摘要；默认/推荐“停止发布”，另一个选项为
     “仍要继续发布”。选择停止时不调用 `fill-publish`、`click-publish` 或 `save-draft`，返回
     保留的发布包。
   - `risk_level == medium`：同时展示风险摘要，使用正常的发布/取消选项。
7. 用户确认后使用 `fill-publish` 或 `fill-publish-video` 填写表单。
8. 填写成功后直接调用 `click-publish`；不要再次等待浏览器预览确认。
9. 用户在最终发布确认时取消：表单尚未填写，不调用 `save-draft` 或其他发布 CLI，返回已保留的发布包。
...
- 最终发布确认时取消：不调用 CLI，保留发布包。
- 表单已经填写后因后续技术失败或用户要求停止：仅在页面内容完整且发布结果并非未知时，
  最多调用一次 `save-draft`。
- 发布前 `check-risk` 返回 `high`：手动对话把风险信息合并到唯一一次最终发布确认且默认停止；
