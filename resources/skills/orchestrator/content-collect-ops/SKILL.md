...
- 在已发布 Run 上回收指标和评论数据，并只写本 Run 的隔离证据。
- 不执行点赞、收藏、评论或回复，不创建发布记录，不维护跨 Run 长期规律。
...
- 本 Run 的指标或评论采集结果、来源、采集时间、阶段状态与失败原因。
- 可供 `content-review-ops` 消费的只读证据；不直接形成长期结论。

## 授权

- 读取已发布内容的指标与评论不属于真实互动，无需发布或互动授权。
- 本 Skill 禁止执行任何产生外部互动的命令；需要评论或回复时必须另行交给互动 Skill 获取授权。
