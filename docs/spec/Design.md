...
- 顶栏使用常驻浏览器图标作为内置浏览器的唯一显式入口，不使用“打开右侧面板”这类容器语义；没有活动会话时按钮保持原位并禁用，提示用户先开始对话。
- 浏览器图标在面板关闭或正在预览文档时切换到内置浏览器；浏览器已展示时再次点击折叠面板，并通过 active 状态表达当前内容。
- 文档不提供空入口，只从会话消息中的内容产物卡片打开；浏览器和文档复用右侧容器、调整宽度、最大化与折叠控制，但各自保持独立入口。
- 右侧面板右上角的折叠按钮是浏览器和文档共用的主折叠入口；点击工作区非交互空白区域可继续作为快捷折叠方式。
- 打开浏览器时必须明确展示加载、失败与重试状态，不允许用空白面板表达浏览器运行时尚未创建或恢复失败。
