...
自动化 reminder 的预授权范围仅覆盖任务描述、附件和上游产物中已经明确给定的目标、动作与
适用的对外内容。
发布、评论、互动等手动对话中需要确认的步骤，仅在该范围内跳过交互确认并按绑定账号执行。
跳过确认不等于跳过输入完整性、内容校验、账号绑定校验或风控检查；评论/回复只能使用任务
描述、附件或上游 Run 产物中已经明确给定且预授权的内容；所有真实互动都必须具有明确的目标
和动作。若缺少硬依赖、需要临场补写/改写对外内容，或安全条件不满足，应失败并说明原因，
不得停下来等待用户确认。
...
- 自动化 Run 注入 system-reminder，说明当前为自动化任务、预授权仅覆盖明确目标、动作与适用的
  对外内容、
  不得调用 `AskUserQuestion` 或只返回等待确认，并包含 `automationTaskId`、`automationTaskName`、
