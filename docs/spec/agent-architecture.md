...
## Agent、Skill 与 Tool 分层

当前产品只保留一个默认业务 Agent：`orchestrator`（内容运营总监）。Agent、Skill、Tool
分别回答不同层次的问题：

| 层级 | 回答的问题 | 负责内容 | 不负责内容 |
| --- | --- | --- | --- |
| Agent | 能做什么、业务流程是什么 | 识别用户目标、选择业务阶段、组织阶段交接、维护跨阶段业务原则 | 工具参数、平台命令、校验阈值、重试与异常分支 |
| Skill | 这个阶段具体怎么做 | 单阶段 SOP、输入输出、工具顺序、校验、授权、失败处理和下一阶段交接 | 定义跨产品的完整业务角色、实现原子工具 |
| Tool | 执行哪个确定操作 | 原子、语义清晰、可验证的读写或平台操作 | 降级、重试、审批决策和跨步骤编排 |

内容运营总监支持账号定位、账号诊断、内容研究、策略与选题、内容生成、发布、互动、
数据采集和复盘。标准业务阶段为：

`账号基础 → 研究与策略 → 内容生成与校验 → 发布 → 互动与采集 → 复盘沉淀`

Agent 根据用户要求从合适阶段开始；用户要求发布、完整流程或运营闭环时，必须继续完成阶段
交接，不能在生成发布包后无故停止。用户明确只要草稿、建议或发布包时，在对应阶段结束。

### 跨阶段不变量

- Profile 表示用户确认的账号事实，优先于历史表现和经验规律；单篇表现不能直接改变 Profile。
- 所有指标、来源、案例和平台结果必须来自用户输入或实际工具结果；缺失数据保持缺失。
- 只有平台明确返回发布成功后，才能进入发布后采集；失败或结果未知不得按成功处理。
- 各阶段只修改自己拥有的数据；长期证据规律由聚合复盘阶段统一维护。
- 产生外部副作用的阶段必须遵守下述授权所有权，前置阶段不得代替其确认。

### 授权所有权

授权采用“Agent 声明政策、末端 Skill 单点执行”的方式：

- Agent Prompt 只声明业务政策：手动对话中的对外发布和真实互动必须获得用户对最终目标与
  动作的授权，发布、评论和回复还必须覆盖最终对外内容；公众号草稿箱推送不属于正式发布，
  当前无需该授权；自动化只能执行任务描述、附件或上游产物中已经明确给定并预授权的目标、
  动作与对外内容。
- 最终产生副作用的 Skill 是唯一授权执行方，负责确认时点、展示字段、交互方式、取消、
  自动化预授权校验和失败分支。生成或研究 Skill 不得提前询问发布或互动确认。
- Runtime reminder 只注入本轮事实，例如运行模式、自动化任务与预授权状态、已选账号和连接器
  能力；不得复制完整业务流程或阶段 SOP。
- 动态事实可以携带只在该上下文成立时才必要的 runtime guard。例如选中小红书账号后注入
  CLI 路由、登录态和账号一致性约束；平台命令、风控、重试和失败分支仍归 Skill。
- 用户显式选择的 Skill 通过 reminder 表示“优先参考”：任务适用时优先采用，不适用时允许
  跳过，也不构成 Skill 白名单。是否继续调用其他阶段 Skill 由原始用户目标和业务流程决定。
- 本阶段仍是 Prompt/Skill 软约束。发布和互动工具尚不校验不可伪造的授权凭据，因此模型直接
  绕过 Skill 调用工具时没有代码级安全保证；runtime 硬授权作为独立后续设计。

### 阶段 Skill 契约

每个阶段 Skill 必须清楚表达以下信息，标题可按领域调整，但语义不得缺失：

1. **职责**：本阶段负责什么、不负责什么，以及副作用所有权。
2. **输入与输出**：消费的账号上下文或阶段产物，以及交给下游的标准产物。
3. **执行流程**：阶段 SOP 和工具调用顺序。
4. **校验与授权**：输入完整性、平台限制、风险检查；无副作用阶段明确标记无需外部动作授权。
5. **失败与交接**：取消、失败、结果未知的处理，以及何时交给下一阶段。

阶段 Skill 之间不建立代码依赖，通过发布包、Run、研究结果等标准产物交接。用户可以替换
单个阶段 Skill，而无需替换完整业务 Agent。
