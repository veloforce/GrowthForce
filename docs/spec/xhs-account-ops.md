...
5. 发布
   ...
   - 预检不自动截断正文、删除图片或丢弃标签。
   - `xhs-publish` 完成风控检查后，用 `AskUserQuestion` 展示最终发布包并执行唯一一次结构化确认。
     `risk_level=high` 时不得先做独立风险确认；必须把高风险原因和风险摘要合并到这次最终确认，
     默认/推荐选项为“停止发布”，另一个选项为“仍要继续发布”。
   - 用户确认后使用 `fill-publish` 或 `fill-publish-video` 填表，成功后直接调用 `click-publish`，不得再次确认。
   - 最终发布确认时取消，表单尚未填写，不调用 `save-draft`；填表后发生确定技术失败或用户要求
...
- `orchestrator` Agent：识别用户目标并组织热点、竞品、选题、草稿、发布和复盘的阶段交接。
- `content-research-ops`、`content-strategy-ops`、`content-review-ops`：提供平台无关阶段能力。
- `xhs-publish`：独占发布确认，负责确认后的填表、发布、取消处理和发布记录。
- `xhs-interact`：明确互动前必须从 feed 获取 `id` 和 `xsecToken`；评论、回复、点赞、收藏及其
  取消操作都由本 Skill 执行唯一一次最终授权。

## 当前架构：Agent 总控与阶段 Skill

平台无关的跨阶段业务流程由 `orchestrator` Agent 负责。Agent 基于用户目标选择起始阶段、
传递标准产物并维护跨阶段不变量，不承载平台命令、工具参数、重试或失败分支。

研究、策略、生成、发布、互动、采集和复盘分别由阶段 Skill 执行；原
`content-lifecycle-ops` 总控 Skill 已解散，避免在 Skill 层重复定义完整业务角色和流程。
长期数据继续使用平台无关的 `content_ops_data` 工具维护。创作者后台数据 CLI 已提供
`creator-account-stats`、`creator-note-stats` 和 `creator-data-snapshot`；长期规律只能基于这些
命令和公开页面实际返回的数据、权限缺口及用户反馈，未返回字段不得补成 0。

`orchestrator` 的跨阶段职责包括：

- 接收账号定位、目标用户、内容方向、竞品账号、关键词和发布目标。
- 选择并衔接探索、研究、策略、创作、发布和复盘阶段。
- 让最终副作用 Skill 独占发布或互动授权，上游阶段不提前确认。
- 产出下一轮运营建议。

详细分层与授权所有权见 `docs/spec/agent-architecture.md`，长期运营闭环见
`docs/spec/content-ops-loop.md`。
...
- Agent 只使用 `creator-*` 命令实际返回的浏览量、曝光数、观看数和创作中心私有数据，
  并明确权限或数据门槛缺口。
- 手动对话中的发布以及评论、回复、点赞、收藏等真实互动都必须经过末端 Skill 的唯一一次确认；
  自动化任务仅可跳过明确预授权目标、动作与对外内容的交互确认。
